...
public class ES93GenericFlatVectorsFormat extends AbstractFlatVectorsFormat {
     ...
     * HNSW builds its graph by random access over the merged raw vectors as soon as the merge has written them, so
     * a direct write would only make that read-back cold. Nothing reads a flat type's merged file back during the
     * merge, so those formats keep direct writes and accept a cold first search. Merge reads of the sources stay direct.
     */
