...
public class DirectIOIT extends ESIntegTestCase {
    ...
    public void testDirectIOUsedForMerges() {
        assumeTrue(
            "the merge create is the same for all three types and is asserted per type in the mapper and format tests; "
                + "bbq_hnsw is the one whose merge also reads its sources with direct I/O",
            type.equals("bbq_hnsw")
        );
        try (MockLog mockLog = MockLog.capture(FsDirectoryFactory.class)) {
